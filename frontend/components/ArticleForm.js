...
export default function ArticleForm(props) {
  ...
  const onSubmit = evt => {
    ...
    if (currentArticle) {
      updateArticle(currentArticle.article_id, {article_id: currentArticle.article_id, ...values});
    } else {
      postArticle(values);
    }
    resetForm(evt);
  ...
  const isDisabled = () => {
    ...
    if (values.text.trim().length >= 1 && values.text.trim().length >= 1 && values.topic !== '') {
      return false;
    }
    return true;
  }
